...
/**
 * This is part of the server side model (REST resource class is just a
 * connection to the model, all logic on server)
 *
...
public enum PersonRegistry {
    ...
    public void create(Person p) {
        if (data.contains(p)) {
            throw new IllegalArgumentException("Person already exists with id " + p.getId());
        }
        data.add(p);
    }
